...
def start_scoreboard(arg):
    global quit, restart, show_scoreboard;
    quit = restart = show_scoreboard = True;
...
while not quit:
    ...
    while True:
        ...
        scoreboard_text = canvas.create_text(canvas.winfo_width() / 2, canvas.winfo_height()-32, text='Scoreboard', state='hidden', font=("Courier", 22));
    ...
    #after game text
    canvas.tag_bind(scoreboard_text, "<Button-1>", start_scoreboard);
    canvas.itemconfig(scoreboard_text, state='normal');
    ...
    #check if user wants to see scoreboard
    if show_scoreboard:
        scoreboard = score.scoreboard(canvas);
        while True:
            scoreboard.draw();
            tk.update_idletasks();
            tk.update();
